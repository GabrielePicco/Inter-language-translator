...
class EnglishToItalianTranslator:
    ...
    def __formula_to_sentece_plan(self, formula):
        ...
        if s:
            ...
            if len(clause_t) == 3:
                return self.__get_ternary_clause(clause_t[0], clause_t[1], clause_t[2], formula)
            if len(clause_t) == 2:
                return self.__get_binary_clause(clause_t[0], clause_t[1], formula)
        else:
    ...
    def __get_exist_clause(self, formula):
        ...
        if exist:
            clause = self.__get_ternary_clause(None, "c'è", exist.group(1), formula)
        return clause
